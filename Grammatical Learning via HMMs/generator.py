...
class HMM_TxtGenerator:
    ...
    def forwards(self, x):
        ...
        for i, observation in enumerate(x):
            if i == 0:
                alpha = B[:,observation] * previous_alpha
            else:
                alpha = B[:,observation] * (A.transpose() @ previous_alpha)
            alphas.append(alpha)
    ...
    def backwards(self, x):
        ...
        x.reverse()
        betas = []
        previous_beta = np.ones((K))
        betas.append(previous_beta)
        for observation in x:
            beta = A @ (B[:,observation] * previous_beta)
            betas.append(beta)
            previous_beta = beta
        betas = betas[:-1]
        betas.reverse()
    ...
    def generate_sentence(self, sentence_length):
        ...
        A = self.model_params.A  # [KxK]
        B = self.model_params.B  # [KxV]
        pi = self.model_params.pi.flatten()  # [Kx1] ------
